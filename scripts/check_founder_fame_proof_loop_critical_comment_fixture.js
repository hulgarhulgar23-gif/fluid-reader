...
async function runFixtureSuite() {
  const script = extractStepScript(
    workflowPath,
    'Reconcile founder fame proof loop critical escalation comment'
  );
  ...
  assertCondition(
    ...
    'Expected founder critical comment action=created when no prior marker comment exists.'
  );
  assertCondition(
    createdScenario.outputs.critical_comment_policy_reason === 'created-no-existing-comment',
    'Expected founder created policy reason when no prior marker comment exists.'
  );
  assertCondition(
    createdScenario.outputs.critical_comment_cooldown_hours === '24' &&
      createdScenario.outputs.critical_comment_min_failure_delta === '2',
    'Expected founder cooldown/min-delta outputs to reflect parsed policy inputs.'
  );
  assertCondition(
    createdScenario.outputs.critical_comment_failure_delta === '5',
    'Expected founder failure delta to equal current failure count on create.'
  );
  assertCondition(
    countMarkerComments(createdScenario.comments) === 1,
    'Expected one founder critical marker comment to be created.'
  );
  ...
  assertCondition(
    ...
    'Expected founder cooldown-delta-skip when within cooldown and failure delta is below threshold.'
  );
  assertCondition(
    cooldownSkipScenario.outputs.critical_comment_cooldown_active === 'true',
    'Expected founder cooldown flag true when update is skipped inside cooldown window.'
  );
  assertCondition(
    cooldownSkipScenario.outputs.critical_comment_policy_reason ===
      'within-cooldown-and-delta-below-threshold',
    'Expected founder cooldown skip policy reason.'
  );
  assertCondition(
    cooldownSkipScenario.outputs.critical_comment_failure_delta === '1',
    'Expected founder failure delta to be computed from existing comment failure count.'
  );
  assertCondition(
    countMarkerComments(cooldownSkipScenario.comments) === 1,
    'Expected duplicate founder critical marker comments to collapse to one during cooldown skip.'
  );
  assertCondition(
    cooldownSkipPrimary &&
      String(cooldownSkipPrimary.body || '').includes('- Failure count this week: 5'),
    'Expected founder primary critical comment body to remain unchanged when cooldown skip applies.'
  );
  ...
  assertCondition(
    ...
    'Expected founder action=updated when failure delta reaches threshold inside cooldown.'
  );
  assertCondition(
    cooldownUpdateScenario.outputs.critical_comment_policy_reason ===
      'updated-within-cooldown-delta-threshold-met',
    'Expected founder within-cooldown update policy reason when failure delta threshold is met.'
  );
  assertCondition(
    cooldownUpdateScenario.outputs.critical_comment_failure_delta === '3',
    'Expected founder failure delta to capture increase over previous comment count.'
  );
  assertCondition(
    cooldownUpdated && String(cooldownUpdated.body || '').includes('- Failure count this week: 8'),
    'Expected founder primary critical comment body to refresh with new failure count after update.'
  );
  ...
  assertCondition(
    ...
    'Expected founder action=updated when cooldown window has expired.'
  );
  assertCondition(
    cooldownExpiredScenario.outputs.critical_comment_policy_reason === 'updated-cooldown-expired',
    'Expected founder cooldown-expired policy reason outside cooldown window.'
  );
  assertCondition(
    cooldownExpiredScenario.outputs.critical_comment_failure_delta === '1',
    'Expected founder failure delta to remain available when cooldown has expired.'
  );
  assertCondition(
    cooldownExpiredUpdated &&
      String(cooldownExpiredUpdated.body || '').includes('- Failure count this week: 9'),
    'Expected founder updated critical comment body after cooldown expiry.'
  );
  ...
  assertCondition(
    ...
    'Expected founder stale critical comments to be cleared when escalation is no longer required.'
  );
  assertCondition(
    clearedScenario.outputs.critical_comment_policy_reason === 'not-escalated-cleared-comments',
    'Expected founder cleared policy reason when stale marker comments are deleted.'
  );
  assertCondition(
    countMarkerComments(clearedScenario.comments) === 0,
    'Expected founder critical marker comments to be fully removed after de-escalation.'
  );
  ...
  assertCondition(
    ...
    'Expected founder action=not-present when no stale comments exist after de-escalation.'
  );
  assertCondition(
    notPresentScenario.outputs.critical_comment_policy_reason ===
      'not-escalated-no-comment-present',
    'Expected founder not-present policy reason when no marker comments are available to clear.'
  );
}

runFixtureSuite()
  .then(() => {
    process.stdout.write('Founder proof-loop critical-comment fixture checks passed.\n');
  })
  .catch((error) => {
    process.stderr.write(`Founder proof-loop critical-comment fixture checks failed: ${error.message}\n`);
    process.exit(1);
  });
